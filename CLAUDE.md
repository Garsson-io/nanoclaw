...
7a. **TDD: write the failing test FIRST, then fix the code.** Never fix a bug without first writing a test that reproduces it and fails. The test proves the bug exists, and then proves the fix works. This applies to all bug fixes and behavioral changes — not just new features. Skipping the failing test means you can't verify the fix is correct.
7b. **Refactor for testability.** When logic is buried in a large function and can't be unit-tested in isolation, extract it into a separate module with a clear interface before writing tests. Untestable code is unverifiable code — refactoring for testability is always justified, never optional.
