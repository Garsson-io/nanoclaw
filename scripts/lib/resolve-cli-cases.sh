#!/bin/bash
# resolve-cli-cases.sh — Resolve the best way to run cli-cases
#
# Solves the chicken-and-egg problem (kaizen #197): fresh worktrees don't have
# dist/ so `node dist/cli-cases.js` fails. This resolver tries tsx from source
# first (always fresh, no build needed), then falls back to compiled dist/.
#
# Usage (executable — preferred, one-liner):
#   CLI_CASES=$("path/to/resolve-cli-cases.sh")                  # auto-detect worktree
#   CLI_CASES=$("path/to/resolve-cli-cases.sh" "/path/to/root")  # explicit root
#   $CLI_CASES case-list
#
# Usage (source then call — still supported):
#   source "path/to/resolve-cli-cases.sh"
#   CLI_CASES=$(resolve_cli_kaizen "/path/to/project")
#   $CLI_CASES case-list

# Resolve cli-cases executable for a given project root.
...
resolve_cli_kaizen() {
  ...
  local dist_js="$project_root/dist/cli-cases.js"
  if [ -f "$dist_js" ]; then
    echo "node $dist_js"
    return 0
  fi
...
# Resolve cli-cases for use in a worktree context.
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  if [ -n "${1:-}" ]; then
    resolve_cli_kaizen "$1" || { echo "error: cannot resolve cli-cases in $1" >&2; exit 1; }
  else
    resolve_cli_kaizen_for_worktree || { echo "error: cannot resolve cli-cases (no worktree or main checkout found)" >&2; exit 1; }
  fi
fi
