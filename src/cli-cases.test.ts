...
import type { CaseCreateDeps, CaseQueryDeps } from './cli-cases.js';
import type { Case } from './cases.js';

const exec = promisify(execFile);
const CLI_SOURCE = path.resolve(__dirname, 'cli-cases.ts');

// INVARIANT: CLI wrapper parses arguments and delegates to domain model functions
// SUT: cli-cases.ts (run via tsx for CI compatibility)
...
describe('cli-cases argument parsing', () => {
  ...
  test('rejects removed commands (list, view)', async () => {
    try {
...
describe('cli-cases case query argument parsing', () => {
