...
describe('handleCaseCreate with existing worktree', () => {
  test('reuses existing worktree when --branch-name and --worktree-path provided', async () => {
    const deps = makeDeps({
      resolveWorktree: vi.fn().mockReturnValue({
        workspacePath: '/existing/worktree',
        worktreePath: '/existing/worktree',
        branchName: 'my-existing-branch',
      }),
    });
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    ...
    expect(deps.resolveWorktree).toHaveBeenCalledWith(
      '/existing/worktree',
      'my-existing-branch',
    );
    expect(deps.createWorkspace).not.toHaveBeenCalled();
  ...
  test('falls back to createWorkspace when resolve returns null', async () => {
    const deps = makeDeps({
      resolveWorktree: vi.fn().mockReturnValue(null),
    });
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    ...
    expect(deps.resolveWorktree).toHaveBeenCalled();
    expect(deps.createWorkspace).toHaveBeenCalled();

    logSpy.mockRestore();
  });

  test('ignores --branch-name without --worktree-path', async () => {
    const deps = makeDeps();
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
  ...
  test('ignores --worktree-path without --branch-name', async () => {
    const deps = makeDeps();
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
