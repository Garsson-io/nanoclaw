#!/usr/bin/env node
/**
 * CLI wrapper for case management operations.
 * Used by skills (markdown prompts) and hooks that need to call the domain model from bash.
 *
 * Usage:
 *   npx tsx src/cli-cases.ts case-create --description "..." --type dev [--github-issue N] [--name "..."]
 *     [--worktree-path PATH --branch-name BRANCH]  (adopt existing worktree)
 *   npx tsx src/cli-cases.ts case-list [--status S1,S2] [--type dev|work]
 *   npx tsx src/cli-cases.ts case-by-branch <branch-name>
 *   npx tsx src/cli-cases.ts case-update-status <name> <status>
 */
...
import { DEV_CASE_ISSUE_REPO } from './github-api.js';
import {
  createCaseWorkspace,
  createCasesSchema,
...
async function main(): Promise<void> {
  ...
  if (!command || command === '--help' || command === '-h') {
    console.error('Usage:');
    console.error(
      '  npx tsx src/cli-cases.ts case-create --description "..." --type dev [--github-issue N] [--name "..."] [--branch-name B --worktree-path P] [--new-worktree]',
    );
    console.error(
      '  npx tsx src/cli-cases.ts case-list [--status S1,S2] [--type dev|work]',
    );
    console.error('  npx tsx src/cli-cases.ts case-by-branch <branch-name>');
    console.error(
      '  npx tsx src/cli-cases.ts case-update-status <name> <status>',
    );
    process.exit(1);
  ...
  if (command === 'case-create') {
    ...
    console.error(
      'Available commands: case-create, case-list, case-by-branch, case-update-status',
    );
    process.exit(1);
  }
}
...
const isDirectRun =
  process.argv[1]?.endsWith('cli-cases.js') ||
  process.argv[1]?.endsWith('cli-cases.ts');
if (isDirectRun) {
  main().catch((err) => {
